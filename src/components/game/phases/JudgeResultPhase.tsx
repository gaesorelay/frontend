...
const JudgeResultPhase = () => {
  ...
  const realJudges = useMemo(() => {
    // 데이터가 없으면 빈 배열
    if (!voteResult?.aiJudges || !roundData?.judges) return [];
    ...
    return voteResult.aiJudges.map((serverJudge, index) => {
      ...
      const originJudge = (roundData.judges as any[])[index];
  ...
  useEffect(() => {
    if (introPhase === 4) {
      ...
      const interval = setInterval(() => {
        let done = true;
        if (curA < publicA) {
          curA += Math.ceil(publicA / 50) || 1;
          if (curA > publicA) curA = publicA;
          done = false;
        }
        if (curB < publicB) {
          curB += Math.ceil(publicB / 50) || 1;
          if (curB > publicB) curB = publicB;
          done = false;
        }
    ...
    if (introPhase === 7) {
      ...
      const interval = setInterval(() => {
        let done = true;
        if (curAiA < aiTotalA) {
          curAiA += Math.ceil(aiTotalA / 50) || 1;
          if (curAiA > aiTotalA) curAiA = aiTotalA;
          done = false;
        }
        if (curAiB < aiTotalB) {
          curAiB += Math.ceil(aiTotalB / 50) || 1;
          if (curAiB > aiTotalB) curAiB = aiTotalB;
          done = false;
        }
        setStageAIA(curAiA);
        setStageAIB(curAiB);
    ...
    >
      ...
      {[4, 7].includes(introPhase) && (
        <img
          ...
          style={{
            ...
            width: 'auto',
            zIndex: 20001,
            filter: 'drop-shadow(0.5vmin 0.5vmin 0 #000)',
            objectFit: 'contain',
          }}
        />
      )}
      ...
      <div className={`intro-overlay ${[1, 2, 3, 5, 6, 8].includes(introPhase) ? '' : 'hidden'}`}>
        ...
        {introPhase === 1 && (
          <img
            src={finalLogo}
            style={{ width: '50vmin', objectFit: 'contain', animation: 'elastic-zoomies 0.8s' }}
          />
        )}
        ...
        {(introPhase === 2 || introPhase === 3) && (
          <div
            ...
            style={{
              textAlign: 'center',
              animation: introPhase === 2 ? 'slide-in-left 0.5s both' : 'slide-in-right 0.5s both',
            }}
          >
            <div style={{ marginBottom: '-3vmin' }}>
              <img
                src={introPhase === 2 ? AteamLogo : BteamLogo}
                style={{
                  width: '60vmin',
                  objectFit: 'contain',
                  filter: 'drop-shadow(0.5vmin 0.5vmin 0 #000)',
        ...
        {(introPhase === 5 || introPhase === 6) && (
          <div
            key={`judge-${introPhase}`}
            style={{
              textAlign: 'center',
              width: '100%',
              animation: 'zoom-in-judge 0.5s cubic-bezier(0.175, 0.885, 0.32, 1.275) both',
              display: 'flex',
              flexDirection: 'column',
              alignItems: 'center',
          ...
          >
            ...
            >
              <img
                src={introPhase === 5 ? ateamImg : bteamImg}
                style={{ height: '12vmin', objectFit: 'contain' }}
              />
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      boxShadow: '0.8vw 0.8vw 0 rgba(0,0,0,0.2)',
                      position: 'relative',
                    }}
                  >
                    <img
                    ...
                    <div style={{ textAlign: 'center', width: '100%' }}>
                      ...
                      <div
                        style={{
                          ...
                          wordBreak: 'keep-all',
                          lineHeight: '1.4',
                          animation: `pop-comment 0.5s ${0.3 + i * 0.2}s both`,
                          fontWeight: '600',
                        }}
                      >
                        {pComment}
                    ...
                    <div
                      style={{
                        ...
                        boxShadow: '0.4vw 0.4vw 0 rgba(0,0,0,0.3)',
            ...
            <div
              style={{
                ...
                marginTop: '4vh',
                fontWeight: 900,
                textShadow: '0.3vw 0.3vw 0 #000',
                animation: 'elastic-zoomies 0.5s 1.5s both',
              }}
            >
      ...
      <div style={{ flex: 1, display: 'flex' }}>
        <div
          style={{
            ...
            justifyContent: 'center',
            position: 'relative',
            opacity: [4, 7].includes(introPhase) ? 1 : 0,
            transition: '0.5s',
          }}
        >
          <div style={{ display: 'flex', gap: '6vmin', padding: '2vh 0 0 2vmin' }}>
            ...
            <div style={{ textAlign: 'center' }}>
              <img
                src={teamALogo}
                style={{ width: '25vw', maxWidth: '300px', objectFit: 'contain' }}
              />
              ...
              <div
                ...
                style={{ width: '22vw', margin: '0 auto', height: '3vmin' }}
              >
                <div
                  className="gauge-fill"
                  style={{
                    width: `${((stagePublicA + stageAIA) / (totalA + totalB || 1)) * 100}%`,
                    background: '#ff4444',
                  }}
                />
                <div
                  style={{
                    ...
                    left: 0,
                    height: '100%',
                    width: `${(stagePublicA / (totalA + totalB || 1)) * 100}%`,
                    background: '#ff7f7f',
                    opacity: 0.8,
                  }}
                />
            ...
            <div style={{ textAlign: 'center' }}>
              ...
              >
                <div
                  className="gauge-fill"
                  style={{
                    width: `${((stagePublicB + stageAIB) / (totalA + totalB || 1)) * 100}%`,
                    background: '#3b82f6',
                  }}
                />
                <div
                  style={{
                    ...
                    left: 0,
                    height: '100%',
                    width: `${(stagePublicB / (totalA + totalB || 1)) * 100}%`,
                    background: '#7fb2ff',
                    opacity: 0.8,
                  }}
                />
            ...
            >
              <button
                ...
                style={{
                  ...
                  color: '#fff',
                  cursor: 'pointer',
                  boxShadow: '0.5vmin 0.5vmin 0 rgba(0,0,0,0.5)',
                  transition: '0.2s',
                }}
