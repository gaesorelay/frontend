...
const JudgeResultPhase = () => {
  const [resultData, setResultData] = useState<VoteResult | null>(null);
  ...
  const { voteResult } = useGameStore();

  useEffect(() => {
    if (voteResult) {
      console.log("📊 [JudgeResultPhase] 스토어의 결과 데이터 사용:", voteResult);
      setResultData(voteResult);
    }
  }, [voteResult]);
